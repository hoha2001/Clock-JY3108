...
static uint8_t rtc2_store_field(uint8_t field, uint8_t val){
  switch(field){
	...
	// *********** Changed, don't correct work with 24 format with hours 20 and more
	  if (val & RTC2_FORMAT_AM)														// test for format time bit 7 = 0 - 24 or 1 - 12
...
static uint8_t rtc2_parse_val(uint8_t field, uint8_t val){
  switch(field){
    ...
    case RTC2_HOURS_READ:
	  ...
	  // ******* changed, not correct works with 24 format
	  if (val & RTC2_FORMAT_AM)
...
void rtc2_get(rtc2_datetime ptr, uint8_t fields){
  ...
  if(fields & RTC2_ALL_FIELDS){
	...
	// ********* changed, no correct work with 24 format
	if (tmp & RTC2_FORMAT_AM)
	{
